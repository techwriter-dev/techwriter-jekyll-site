...
* *PHP variable names* follow the same rules as PHP application script filenames: are all lowercase with underscores, for example, `$parent_id_column`
* Use lowercase forms of *reserved keywords* unless PHP documentation uses caps, for example, `true | false | null | self  `
...
In fact, due to the highly collaborative requirements of scrum, the QA analyst should have less work to do in the trenches since a good deal of testing should have already been performed by the developers, especially regarding link:#unit_testing[unit testing].
...
to UTF-8 encoding`"] asked by user "`Sam`" on Stack Overflow, 6 Jan 2010.
